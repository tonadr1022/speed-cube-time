...
func RegisterHandlers(r *mux.Router, service Service, authHandler util.MiddlewareFunc) {
	...
	r.HandleFunc("/settings/{id}", authHandler(util.MakeHttpHandler(res.getById))).Methods(http.MethodGet, http.MethodOptions)
	r.HandleFunc("/users/{userId}/settings", authHandler(util.MakeHttpHandler(res.getByUserId))).Methods(http.MethodGet, http.MethodOptions)
	r.HandleFunc("/settings", authHandler(util.MakeHttpHandler(res.query))).Methods(http.MethodGet, http.MethodOptions)
	r.HandleFunc("/settings", authHandler(util.MakeHttpHandler(res.create))).Methods(http.MethodPost, http.MethodOptions)
	r.HandleFunc("/settings/{id}", authHandler(util.MakeHttpHandler(res.update))).Methods(http.MethodPatch, http.MethodOptions)
	r.HandleFunc("/settings/{id}", authHandler(util.MakeHttpHandler(res.delete))).Methods(http.MethodDelete, http.MethodOptions)
}
